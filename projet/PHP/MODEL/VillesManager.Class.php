...
class VillesManager 
{
	...
	{
 		$db=DbConnect::getDb();
		$q=$db->prepare("INSERT INTO Villes (nomVille,numDept,codePostal) VALUES (:nomVille,:numDept,:codePostal)");
		$q->bindValue(":nomVille", $obj->getNomVille());
		$q->bindValue(":numDept", $obj->getNumDept());
	...
	{
 		$db=DbConnect::getDb();
		$q=$db->prepare("UPDATE Villes SET idVille=:idVille,nomVille=:nomVille,numDept=:numDept,codePostal=:codePostal WHERE idVille=:idVille");
